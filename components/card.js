...
export default ({ cardImage, cardTitle, cardCaption, cardHref, cardClasses, cardSource }) => {
		...
		<article className={`opacity-0 ${cardClasses.join(' ')} ${useIsInViewport(cardRef) ? 'fade-in' : null}`} ref={cardRef}>
			<div className='card-wrapper'>
				...
				<Link href={cardHref} passHref>
					<a
						target={`_${cardSource === null ? 'self' : 'blank'}`}
						rel={cardSource === null ? '' : 'noopener nofollow noreferrer'}
						>
						<div className='card-data'>
							...
							<p className='card-caption'>{cleanString(cardCaption)}</p>
							{cardSource !== 'instagram' && <span className='btn-primary card-link'>View details</span>}
						</div>
					</a>
				</Link>
			</div>
